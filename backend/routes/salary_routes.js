...
} = require("../models/salary_models.js");
const {
  deleteSalary,
  insertSalary,
} = require("../controllers/salary_contoller.js");
...
router.post("/getSalaryDataConvert", async (req, res) => {
  ...
  try {
    const result = await getSalaryDataConvert(id_shelter, month, year);
...
router.post("/insertSalaryData", async (req, res) => {
  try {
    const { id_shelter, id_employee, name, cost, date, note, created_by } =
      req.body;
